...
A small scientific group that needs a defensible knowledge base over its own literature, and whose job is the science rather than the retrieval engineering.

That shapes two things. The architectural decisions are made for you and written down with their reasoning, so you can defend them in a review instead of re-deriving them. And the operational surface is one database, because a group with no platform team pays the cost of a second one every week.
...
That matters for how you specialize it. You edit data and configuration, meaning the `domain/` folder, the corpus manifest, and `.env`, rather than renaming Python packages or filling in placeholders. The thing you read is the thing you run, before and after. [Why a template repository instead of a cookiecutter](#why-a-template-repository-instead-of-a-cookiecutter) has the reasoning.
...
The evidence is numeric and tabular, so the chunker keeps tables intact as their own chunks rather than shredding them across a splitter's window. Questions and documents use different words, so retrieval fuses keyword search and a hypothetical-document probe alongside vector search rather than relying on embeddings alone. And corpora mix redistribution rights, so a license class is a fail-closed property enforced inside every layer's SQL rather than a filter applied afterwards.
...
Different shapes rather than better or worse. LightRAG and LlamaIndex are libraries you build an application around, and they win when you want to make the architectural calls yourself. PaperQA2 is an agent that spends multi-step reasoning per query, and it wins when the question is hard enough to be worth that cost and latency.
...
Clone the repository and run the demo. It ingests five synthetic CC0 documents about agricultural residues, retrieves evidence for one question, and scores that retrieval against the bundled seed questions.
...
That takes about ten minutes and needs no credentials. When you want to start a project rather than evaluate one, `pipx install sci-rag-kit` then `sci-rag-new` runs the wizard instead. The [quickstart](quickstart.md) walks through both.

### Do I need Google credentials?

Not to try it. Not to run retrieval. Yes for anything that calls a model.

With `SCI_RAG_EMBEDDING_PROVIDER=local-hash` the kit uses a deterministic offline embedder, and parsing, chunking, storage, ranking, and retrieval evaluation all work. Similarity is lexical rather than semantic, which is a real quality difference and not a hidden one.
...
Set `SCI_RAG_EMBEDDING_PROVIDER=local-hash` and the whole test suite and the demo run without a network call. What you lose is everything that needs a model: the knowledge graph, community summaries, HyDE, generated answers, and the judge. Asking for an answer in that state reports that no LLM is configured rather than inventing one, which is the same refusal behavior the kit applies when retrieval finds nothing.

## Why it is built this way

This section is the short form of the [decision records](adr/0001-graph-in-postgres.md). Each answer states the decision and its main reason; each record states the alternatives considered, the consequences accepted, and the conditions under which we would reverse it. Reach for the record when you are about to change the decision.

### Why is the knowledge graph in Postgres and not Neo4j?

Because the workload does not need a graph engine, and a second database costs more than it saves.

The only traversal this methodology performs is a two-hop walk from a handful of seed entities, then a jump back to the evidence chunks. That is a recursive query over indexed foreign keys, comfortably fast at the scale a domain corpus produces. Graph engines earn their keep on deep, unbounded traversals the kit never issues. Against that, a second database means a second backup story, a second migration story, a second access-control model, and a consistency seam between the graph and the chunks it references. One database also lets a chunk and its graph entries commit together.

Deep path queries and centrality are out of scope as a result. Export the two tables to a graph tool rather than moving the operational store.

The full argument, including the conditions under which we would reverse it, is in [ADR 0001](adr/0001-graph-in-postgres.md).

### Why 1536-dimension embeddings when models offer 3072?

Because pgvector's HNSW index tops out at 2000 dimensions, and a 3072-dimension column silently falls back to exact scans where every vector query reads every row.

At ten thousand chunks that is fine. Past that it hurts, and it is the kind of slowdown nobody notices until the corpus has already grown. The default model is trained with Matryoshka representation learning, so the first 1536 dimensions carry most of the signal at a small and published cost in quality. Two guardrails come with the truncation. Every truncated vector is re-normalized to unit length, because truncation breaks the unit norm cosine ranking assumes. And the dimension is asserted twice, by the provider and by the database column.

Full 3072 dimensions would retrieve slightly better. Measure that gap with the ablation harness before paying the exact-scan price.

The full argument, including the conditions under which we would reverse it, is in [ADR 0002](adr/0002-embeddings-1536-hnsw.md).

### Why is Docling optional instead of the default?

Because it pulls several gigabytes of machine-learning stack, and a hard dependency that size would make `uv sync` and CI miserable for everyone who never opens a PDF.

Docling is still the recommended PDF route. Install it with `uv sync --extra docling` and the parser uses it automatically when it is importable, because it does genuine layout analysis and table-structure recognition, and tables are where cheap parsers fail on scientific PDFs. Without it, pypdf is the always-available fallback. Both routes plus native Markdown converge on one block model, so nothing downstream has to know which one produced a document.

A corpus ingested with pypdf has worse table fidelity. That is a real difference, and it is why the parser records the route rather than hiding it.

The full argument, including the conditions under which we would reverse it, is in [ADR 0003](adr/0003-docling-with-pypdf-fallback.md).

### Why a template repository instead of a cookiecutter?

Because a cookiecutter template is dead code, and this one has to be inspectable to be trustworthy.

Cookiecutter parameterizes everything, and the price is a tree of placeholders that cannot be browsed comfortably, cannot run, and cannot be tested as itself. Template bugs are found by generating a project and looking. A runnable template inverts that: a newcomer evaluates the kit by reading and running it before committing to it, the demo is the documentation of behavior, and the same CI that gates any application gates template quality.

What is given up is deep parameterization such as license choice and layout variants. This template has opinions, and disagreeing with one costs a normal code change in your own copy.

The full argument, including the conditions under which we would reverse it, is in [ADR 0004](adr/0004-template-repo-not-cookiecutter.md).
...
`kg_relationships` is entity-to-entity by contract: both endpoints are foreign keys into `kg_entities`, the relation type must be one the domain ontology declares, and every row carries the chunk and the quoted phrase that stated it. A citation has none of those. It runs document to document, its type is always the same, and its evidence is a DOI match in a reference list rather than a sentence somebody wrote. The alternative that would preserve the table, a synthetic entity per document, makes entity search return paper titles alongside concepts, and makes community detection summarize the mixture.

So `document_citations` holds them, and only for pairs where both documents are in the corpus. A reference to a paper nobody ingested is a dangling pointer, not a graph edge.

The full argument, including the conditions under which we would reverse it, is in [ADR 0005](adr/0005-citation-edges-as-a-document-table.md).

### Why hand-written provider adapters instead of LiteLLM?

Because the provider differences that matter here are exactly the ones a translation layer would smooth over.

JSON-mode calls set `thinking_budget=0` on Gemini for a documented reason: without it, extraction and judging spend their whole output budget on thought and return empty. The Claude equivalent is `output_config={"effort": "low"}`, a different knob with different semantics rather than a translation of the same one, because disabling thinking on current Claude models can leak reasoning tags into the JSON those call sites parse. Those models also removed the sampling parameters outright, so the adapter omits `temperature` rather than forwarding it.

Three adapters ship: `google`, `anthropic`, and `openai-compatible`. The third covers Grok, Llama, Mistral, DeepSeek, OpenAI itself, and a self-hosted vLLM or Ollama server, because Vertex serves its partner models behind an OpenAI-compatible endpoint rather than native APIs.

The full argument, including the conditions under which we would reverse it, is in [ADR 0006](adr/0006-multi-provider-llms.md).
...
A migration bakes `SCI_RAG_EMBEDDING_DIM` into the pgvector column, and every chunk stores the version that produced it. Changing the embedder therefore means a migration, a full re-embed, and an index rebuild. A provider setting would advertise a configuration change for work that is nothing of the sort. There is also nothing to switch to: Anthropic ships no embedding API, and on Vertex the only managed text embeddings are Google's, so everything else means deploying and paying for a GPU endpoint.

You can point `SCI_RAG_EMBEDDING_MODEL` at a different Google embedding model freely, and `sci-rag embed plan` scopes the re-embedding work when you need more than that.

The full argument, including the conditions under which we would reverse it, is in [ADR 0006](adr/0006-multi-provider-llms.md).

### Why does the generator configure instead of template?

Because a generator that rendered placeholders would drag back exactly what the template decision refused.

`sci-rag-new` downloads this repository at a pinned tag and then rewrites its configuration files in place. There is no `{{ }}` syntax and no `{{ }}` directory anywhere in the tree, and a test asserts that a generated project contains no bare `{{` in any Markdown, TOML, YAML, or JSONL file. What the generator fetches is this repository byte for byte, so the template stays browsable, runnable, and tested as itself. The wizard is ordinary tested code under `src/sci_rag/scaffold/`, and the appliers round-trip their output through the same models the application reads, so a generated profile cannot be something `load_domain()` would reject.

The full argument, including the conditions under which we would reverse it, is in [ADR 0007](adr/0007-interactive-project-generator.md).

### Why support three PostgreSQL majors instead of pinning one?
...
The environment manager work added pixi and conda because scientific and national-lab users asked for them, and those users often cannot install Docker on a managed laptop or a cluster login node. conda-forge ships PostgreSQL and pgvector together, but builds the extension against whatever server it currently carries, which today is 18, while compose and CI run 16. Rather than move everyone to 18, pin those users to a two-year-old extension, or let the two diverge untested, the project supports 16 through 18 and tests both ends. Nothing in the schema needs a particular major: one HNSW index, no pgvector feature newer than 0.5, no version-specific SQL.
...
### Why fuse by rank instead of by score?
...
A `Reranker` protocol ships with two adapters: an LLM one that needs no new dependencies, and a local cross-encoder behind the `rerank` extra. Both stay off until the `with_rerank` against `no_rerank` ablation justifies the latency on the corpus you actually have. That rule is not specific to reranking. It is the same rule contextual compression is held to, and the reason compression is currently off too: its paired judged-answer gate was re-run for v0.3 and did not hold. [Benchmarks](benchmarks.md) publishes both comparisons.
...
Because the alternative is a confident answer built from model priors rather than from your corpus, which is the failure this kit exists to avoid.

The answer prompt carries three standing orders: cite every claim inline by number, prefer the sources' numbers and units over summary, and say so when the sources do not contain the answer. A refusal means retrieval found nothing in scope, and it is more useful than a plausible paragraph you would have to go and check. The evaluation harness keeps at least one question tagged `unanswerable` in every real seed set for exactly this reason, so the honesty behavior is measured rather than assumed.

## Rights and evidence

### Can I use this on papers I hold but may not redistribute?

Yes, and that case is why rights are a first-class property rather than a filter.
...
The same reasoning runs through the corpus campaign path. Discovery produces candidate metadata only, and a missing or unrecognized license signal stays `unknown` rather than being guessed from a title or a failed request.
...
Both run at temperature 0, scores clamp to a 0 to 2 rubric, and a malformed judge response is recorded as a failure rather than coerced into a zero.
...
Two things hold that up. Vector search stays indexed because the default dimension sits inside pgvector's HNSW limit, so it does not degrade into a full scan as the corpus grows. And graph traversal is a two-hop walk over indexed foreign keys rather than an unbounded path query. The decision record's own revisit condition is a corpus reaching millions of entities, or a product that needs three or more hops at query time.

If you get there, the seam is clean: the graph layer is one stage behind the retrieval facade, so swapping in a graph engine means reimplementing one stage rather than the kit.

### What does it cost to run?

Locally, nothing beyond your own machine and whatever model calls you make. Deployed, the database is the steady cost.

The Google Cloud deployment is one Cloud SQL instance with pgvector, one Cloud Run service for REST and MCP, and one Cloud Run job for migrations and ingestion. The database is what runs continuously, and the default `db-g1-small` tier is a few tens of dollars a month, so tear down experiments with `terraform destroy` rather than leaving them up.

Model cost is yours to control, and the profiles are the lever. `interactive` runs vector and keyword only, so the retrieval path costs one query embedding and caches it briefly. `deep` adds graph traversal and HyDE, which each call a generation model per query. There is no per-query agentic loop either way, so cost per question is predictable rather than a function of how hard the model decided the question was.
