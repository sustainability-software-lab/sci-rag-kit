...
Three pages are generated from the source and checked by the build: the CLI page from the command definitions, the configuration page from the settings and domain models, and the benchmarks page from the evaluation reports. The others are written by hand.
