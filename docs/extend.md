...
!!! warning "Partner models are not served from every region"
    ...
    A model that is not served where you asked fails with `400 ... is not servable in region` or `404 ... not found in location`, and `sci-rag doctor --probe` catches it before a pipeline run does. The probe reports the location it actually used and names `SCI_RAG_GCP_LOCATION=global` as the repair. It keeps the two apart: the `400` means the model exists somewhere else, so the location is the only thing to change, while the `404` also covers a wrong model id or an offering this project never enabled, so the probe offers both readings rather than sending you to change a location that was never the problem.
